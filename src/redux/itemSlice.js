  ...
  reducers: {
    setProds: (state, action) => {
      ...
      if (sonuc) {
        ...
        state.prods.filter((item, index) => item.id === state.id ? state.prods[index].quantity = parseInt(state.prods[index].quantity) + 1 : console.log("id eşit değil"))
    ...
    DeleteProd: (state, action) => {
      state.id = action.payload;
      state.prods.filter(item => item.id === state.id ? item.quantity = item.quantity - 1 : console.log("id eşit değil"))

      if (state.prods.filter(item => item.quantity === 0)) {
        state.prods.filter(item => item.quantity === 0 ? state.prods.splice(state.prods.indexOf(item), 1) : "")
      }
      const filtered = productsData.filter((item) => item.id === state.id);
